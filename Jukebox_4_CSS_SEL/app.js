...
$(document).ready(function() {
  ...
  $("li").hover(function(){
      $(this).toggleClass("hover_song");
  });

  $("#library-list, #playlist-list").delegate("i.fa-trash", "click", function(){
    $(this).parents("li").slideUp(500, function(){
      $(this).remove();
    });
  });

  $("#library-list, #playlist-list").delegate("li", "dblclick", function(event){
  ...
  $("#filter-library").change(function(){
    ...
    var value = $("#filter-library").val();
    console.log(value);
    if(value){
      $("#library-list").find("li:not(:contains(" + value + "))").slideUp();
      $("#library-list").find("li:contains(" + value + ")").slideDown();
    }else{
      $("#library-list").find("li").slideDown();
    }
  }).keyup(function(){
    $(this).change();
  });
